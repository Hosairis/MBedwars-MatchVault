...
data class PlayerStatsData(
    ...
    companion object {
        fun read(id: Long): PlayerStatsData? {
            return PlayerStats
                .selectAll()
        ...
        fun readByPlayerId(playerId: Long): PlayerStatsData? {
        ...
        fun update(
            ...
            return PlayerStats.update({ PlayerStats.id eq id }) { statement ->
                val fetchRow = {
                    PlayerStats
                        .selectAll()
                        .where { PlayerStats.id eq id }
        ...
        fun updateByPlayerId(
            playerId: Long,
            builder: UpdateBuilder<Int>.(fetchRow: () -> ResultRow?) -> Unit
        ): Boolean {
            val playerRef = EntityID(playerId, Players)
            return PlayerStats.update({ PlayerStats.playerId eq playerRef }) { statement ->
                val fetchRow = {
                    PlayerStats
                        .selectAll()
                        .where { PlayerStats.playerId eq playerRef }
    ...
    fun update(builder: (UpdateBuilder<Int>.(PlayerStatsData) -> Unit)? = null): Boolean {
        val playerRef = EntityID(playerId, Players)
        return PlayerStats.update({ PlayerStats.playerId eq playerRef }) { statement ->
            if (builder == null) {
                statement[PlayerStats.matchesPlayed] = this@PlayerStatsData.matchesPlayed
    ...
    fun delete(): Boolean {
        val playerRef = EntityID(playerId, Players)
        return PlayerStats.deleteWhere { PlayerStats.playerId eq playerRef } > 0
    }
}
